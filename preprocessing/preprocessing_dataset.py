# -*- coding: utf-8 -*-
"""
Created on Mon Dec 16 13:05:44 2019
...
def time_strip(x):
    try:
        return  int(time.mktime(datetime.strptime(x,"%Y-%m-%d %H:%M:%S.%f%z").timetuple()))
    except:
        return int(time.mktime(datetime.strptime(x,"%Y-%m-%d %H:%M:%S%z").timetuple()))
...
data['completeTime'] = data['completeTime'].apply(time_strip)
